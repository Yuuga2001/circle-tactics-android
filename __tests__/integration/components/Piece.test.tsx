...
import { PLAYER_COLORS, PIECE_SIZE_RATIO, CELL_SIZE } from '../../../src/styles/theme';

describe('Piece', () => {
  it('REDプレイヤーのSMALLピースが表示される', () => {
    const { getByTestId } = render(<Piece player="RED" size="SMALL" testID="piece" />);
    expect(getByTestId('piece')).toBeTruthy();
  });

  it('BLUEプレイヤーのMEDIUMピースが表示される', () => {
    const { getByTestId } = render(<Piece player="BLUE" size="MEDIUM" testID="piece-blue" />);
    expect(getByTestId('piece-blue')).toBeTruthy();
  });

  it('REDプレイヤーのpiececは赤色を持つ', () => {
    const { getByTestId } = render(<Piece player="RED" size="LARGE" testID="piece-red" />);
  ...
  it('BLUEプレイヤーのpiececは青色を持つ', () => {
    const { getByTestId } = render(<Piece player="BLUE" size="SMALL" testID="piece-blue" />);
  ...
  it('SMALLサイズは適切なサイズを持つ', () => {
    const { getByTestId } = render(<Piece player="RED" size="SMALL" testID="piece-small" />);
    const element = getByTestId('piece-small');
    const style = element.props.style;
    const flatStyle = Array.isArray(style) ? Object.assign({}, ...style.map((s: object) => s || {})) : style;
    const expectedSize = CELL_SIZE * PIECE_SIZE_RATIO.SMALL;
    expect(flatStyle.width).toBe(expectedSize);
    expect(flatStyle.height).toBe(expectedSize);
  });

  it('LARGEサイズは適切なサイズを持つ', () => {
    const { getByTestId } = render(<Piece player="GREEN" size="LARGE" testID="piece-large" />);
    const element = getByTestId('piece-large');
    const style = element.props.style;
    const flatStyle = Array.isArray(style) ? Object.assign({}, ...style.map((s: object) => s || {})) : style;
    const expectedSize = CELL_SIZE * PIECE_SIZE_RATIO.LARGE;
    expect(flatStyle.width).toBe(expectedSize);
    expect(flatStyle.height).toBe(expectedSize);
  });

  it('円形（borderRadius）が設定されている', () => {
    const { getByTestId } = render(<Piece player="YELLOW" size="MEDIUM" testID="piece-circle" />);
